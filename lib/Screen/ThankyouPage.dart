...
import 'package:ecommerce/Modal/sendOrderMailModal.dart';
...
class ThankyouPage extends StatefulWidget {
  ...
  ThankyouPage({super.key, this.uid, this.url, this.orderId, this.userId});
...
class _ThankyouPageState extends State<ThankyouPage> {
  ...
  Sendmailorder() async {
    ...
    print('Le beta Deta : $data');
    checkInternet().then((internet) async {
      if (internet) {
        authprovider().sendOrderMailapi(data).then((response) async {
          sendOrderMail =
              SendOrderMailModal.fromJson(json.decode(response.body));
          print(sendOrderMail?.status);
          if (response.statusCode == 200 &&
              sendOrderMail?.status == "success") {
